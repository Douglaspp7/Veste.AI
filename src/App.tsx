// @ts-nocheck
/* STREAMING_CHUNK: Importing dependencies and icons... */
...
/* STREAMING_CHUNK: Defining UI Badge components... */
...
/* STREAMING_CHUNK: Utility mapping functions for UI... */
...
/* STREAMING_CHUNK: Initializing Main App State... */
...
/* STREAMING_CHUNK: Setting up API configurations... */
// APIs Tokens
...
/* STREAMING_CHUNK: Loading localStorage persistence... */
useEffect(() => {
...
/* STREAMING_CHUNK: Creating handle functions for Settings and Shortcuts... */
...
/* STREAMING_CHUNK: Adding Download logic for media and copy... */
...
// Fallback seguro se o Facebook bloquear via CORS
window.open(url, '_blank');
}
};

/* STREAMING_CHUNK: Integrating AI features... */
...
setAiFeedback("Erro: Por favor, adicione a sua Chave API do Gemini nas Configurações da API.");
return;
}
if (aiProvider === 'chatgpt' && !chatGptToken.trim()) {
setAiFeedback("Erro: Por favor, adicione a sua Chave API do ChatGPT nas Configurações da API.");
return;
}
}
...
/* STREAMING_CHUNK: Integrating Data Mining Engine... */
...
if (!token) { setMiningError("Configure o Token da Apify nas Configurações da API."); return; }
...
/* STREAMING_CHUNK: Filtering and rendering UI setup... */
...
/* STREAMING_CHUNK: Main Render DOM... */
return (
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 mb-6">
             ...
             <div className="flex flex-wrap gap-3">
                 {selectedAd.videoUrl && (
                     <button 
                         onClick={() => downloadMedia(selectedAd.videoUrl, `video_${selectedAd.id}.mp4`)} 
                         className="flex-1 min-w-[140px] bg-slate-800 hover:bg-slate-700 text-slate-200 py-2 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors text-sm font-bold border border-slate-700 hover:border-slate-600"
                     >
                         <Video size={16}/> Baixar Vídeo
                     </button>
                 )}
                 {selectedAd.mediaUrl && !selectedAd.videoUrl && (
                     <button 
                         onClick={() => downloadMedia(selectedAd.mediaUrl, `imagem_${selectedAd.id}.jpg`)} 
                         className="flex-1 min-w-[140px] bg-slate-800 hover:bg-slate-700 text-slate-200 py-2 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors text-sm font-bold border border-slate-700 hover:border-slate-600"
                     >
                         <ImageIcon size={16}/> Baixar Imagem
                     </button>
                 )}
                 <button 
                     onClick={() => {
                         const blob = new Blob([selectedAd.copy], { type: 'text/plain;charset=utf-8' });
                         const url = window.URL.createObjectURL(blob);
                         const a = document.createElement('a');
                         a.href = url;
                         a.download = `copy_${selectedAd.id}.txt`;
                         a.click();
                     }} 
                     className="flex-1 min-w-[140px] bg-slate-800 hover:bg-slate-700 text-slate-200 py-2 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors text-sm font-bold border border-slate-700 hover:border-slate-600"
                 >
                     <Copy size={16}/> Baixar Copy (.txt)
                 </button>
             </div>
          </div>
